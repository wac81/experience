...
doc_distance_limit = 0.7

# time area
from_date = 201603000000
to_date = 201605000000
        ...
        for line in f:
            ...
            if fieldnames == None:
                ...
                if set(fieldnames).issubset(set(object_dict.keys())):
                    if (convert_time(object_dict["tC"]["$date"]) > from_date and convert_time(object_dict["tC"]["$date"]) < to_date):
                        one = []
...
datafilename = '/home/wac/data2/Infiniti_0510.json'
# datafilename = './coralqq.json'

documents = json_dict_from_file(datafilename, ['content', 'reposts', '_id'], False)
...
for id, itemone in enumerate(numarray):
    ...
    # print sort_sims
    print "content: " + documents[sort_sims[0][0]][0]
    print "sims: " + str(sort_sims[0][1])
    print "weibo_count: " + str(len(numarray[id]))
    print "reposts_count: " + str(repostsnum[id])
    print "id: " + documents[sort_sims[0][0]][2]
